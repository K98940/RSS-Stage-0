...
const LS_KEY = 'RSS-random-game-202310012127';
...
export const renderScoreBoard = async () => {
  ...
  const htmlHeader = `
    ...
    <thead class="thead bg-white-blur">
      <th width="100%" class="__align_left nickname">
        <button id="btn-change-name" title="change the name">${state.nickname}</button>
      </th>
  ...
  const htmlTable = scores.data.reduce((html, row, i) => {
    ...
    name = name.toLowerCase() === 'wisely' ? `<span style="color: blue">${name}</span>` : name;
    const style = isMyNick ? 'class="score-myscore"' : '';
    const attribLastResult = lastResult === row.id ? ' data-lastresult="yes"' : '';
    const city = row.city ? ` (${row.city})` : ``;

    return `${html}
    <tr title="date of the result: ${row.date}" ${style} ${attribLastResult}>
  ...
  btnChangeName.addEventListener('click', () => {
    showMessage(
      'Tell me your name.\nThe length of the name is limited to 32 characters.\nChoose your name wisely!',
      true
    );
  });
...
export const saveLocalStorage = () => {
  ...
  localStorage.setItem(LS_KEY, ls);
};
